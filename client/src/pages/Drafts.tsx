            ...
            {drafts.map((draft) => (
              <div key={draft.id} className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-100 p-6 flex flex-col justify-between hover:shadow-2xl transition-all duration-200">
                ...
                <div className="flex gap-2 mt-2 flex-wrap">
                  <button
                    className="flex items-center gap-1 px-3 py-2 bg-orange-600 text-white font-semibold hover:bg-orange-700 transition text-sm rounded-sm"
                    onClick={() => handleEdit(draft.id)}
                  >
                    <PencilSquareIcon className="h-4 w-4" /> Edit
                  </button>
                  <button
                    className="flex items-center gap-1 px-3 py-2 bg-green-600 text-white font-semibold hover:bg-green-700 transition text-sm rounded-sm"
                    onClick={() => handleView(draft.id)}
                  >
                    <EyeIcon className="h-4 w-4" /> View
                  </button>
                  <button
                    className="flex items-center gap-1 px-3 py-2 bg-purple-600 text-white font-semibold hover:bg-purple-700 transition text-sm rounded-sm"
                    onClick={() => handleSend(draft)}
                  >
                    <PaperAirplaneIcon className="h-4 w-4" /> Send
                  </button>
                  <button
                    className="flex items-center gap-1 px-3 py-2 bg-red-600 text-white font-semibold hover:bg-red-700 transition text-sm rounded-sm"
